...
export function Pagination({
  ...
  const fullPages = useMemo(
    () => getFullVisiblePages(totalPages),
    [totalPages]
  );
  ...
  return (
    ...
    >
      <PaginationLink
        href={createPaginationHref({ basePath, page: 1, query })}
      ...
      <PaginationLink
        href={createPaginationHref({ basePath, page: totalPages, query })}
    ...
    <span className="flex items-center gap-2">
      {pages.map((page, index) =>
        page === "ellipsis" ? (
          ...
          <PaginationLink
            key={page}
            href={createPaginationHref({ basePath, page, query })}
...
function getPaginationWidth(pagesCount: number) {
  ...
  return itemsCount * PAGINATION_ITEM_SIZE + (itemsCount - 1) * PAGINATION_GAP;
}
