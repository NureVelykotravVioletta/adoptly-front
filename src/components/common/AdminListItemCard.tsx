...
type AdminListItemMeta = {
  label: string;
  value: string;
};

type AdminListItemCardProps = {
  imageUrl: string | null;
  title: string;
  subtitle?: string;
  description: string;
  meta: AdminListItemMeta[];
  editHref: string;
    ...
    <article className="grid w-full min-w-0 items-center gap-5 rounded-[28px] bg-white px-5 py-5 text-[#262626] shadow-[0_8px_24px_rgba(38,38,38,0.04)] md:grid-cols-[132px_minmax(0,1fr)_auto] md:px-7">
      ...
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
          <h2 className="truncate text-xl leading-7 font-bold">{title}</h2>
          {subtitle ? (
