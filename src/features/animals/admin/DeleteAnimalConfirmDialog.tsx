"use client";

import { useCallback, useState, useTransition } from "react";
import { Button } from "@/src/components/common/Button";
import { LoadingOverlay } from "@/src/components/common/LoadingOverlay";
import { ModalDialog } from "@/src/components/common/ModalDialog";
import { deleteAnimalAction } from "@/src/features/animals/animals.action";
import type { Animal } from "@/src/features/animals/animals.api";

type DeleteAnimalConfirmDialogProps = {
  animal: Animal | null;
  onClose: () => void;
  onDeleted: (animalId: string) => void;
};

export function DeleteAnimalConfirmDialog({
  animal,
  onClose,
  onDeleted,
}: DeleteAnimalConfirmDialogProps) {
  const [error, setError] = useState("");
  const [isPending, startTransition] = useTransition();
  const isOpen = Boolean(animal);

  const closeDialog = useCallback(() => {
    if (isPending) {
      return;
    }

    setError("");
    onClose();
  }, [isPending, onClose]);

  if (!animal) {
    return null;
  }

  function handleDelete() {
    if (!animal || isPending) {
      return;
    }

    setError("");

    startTransition(async () => {
      const result = await deleteAnimalAction(animal.id, animal.shelterId);

      if (result.error) {
        setError(result.error);
        return;
      }

      onDeleted(animal.id);
    });
  }

  return (
    <ModalDialog
      isOpen={isOpen}
      titleId="delete-animal-title"
      closeDisabled={isPending}
      onClose={closeDialog}
      className="max-w-[420px] rounded-[28px] px-7 pt-14 pb-8 text-center"
    >
      <h2 id="delete-animal-title" className="text-xl font-bold">
        Видалити тварину?
      </h2>

      <p className="mt-3 text-sm leading-6 text-[#5F5F5F]">
        Тварину «{animal.name}» буде видалено. Цю дію не можна скасувати.
      </p>

      {error ? <p className="mt-4 text-sm text-rose-600">{error}</p> : null}

      <div className="mt-7 grid grid-cols-2 gap-2">
        <Button variant="danger" disabled={isPending} onClick={handleDelete}>
          {isPending ? "Видаляємо..." : "Видалити"}
        </Button>
        <Button variant="neutral" disabled={isPending} onClick={closeDialog}>
          Скасувати
        </Button>
      </div>

      {isPending ? <LoadingOverlay label="Видалення тварини" /> : null}
    </ModalDialog>
  );
}
